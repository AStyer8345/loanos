...
- [ ] **NEEDS ADAM — Reconcile NotebookLM nightly playbook vs scheduled task email rule** (flagged 2026-04-26 PM nightly run): scheduled task `styer-notebooklm-nightly/SKILL.md` says "Do not send any emails to Adam. All reporting goes into project files only." But subagent playbooks `tasks/seo-sem/subagents/00-notebooklm.md` Step 5c + `tasks/lead-gen/subagents/00-notebooklm.md` Step 5c both say to send the digest via Zapier. Tonight followed the playbook before noticing the override; both digests were sent. Pick the rule and remove the contradiction: either (a) delete Step 5c from both playbooks (digests live in `tasks/<area>/digests/` only, no email), or (b) remove the "no emails" sentence from `styer-notebooklm-nightly/SKILL.md`. Until reconciled, future nightly runs will keep hitting this fork.
...
- [ ] **3 active campaigns missing authored content.** Long-Term Nurture (2 steps), Past Client Retention (6 steps), Realtor Relationships (4 steps already exist as `ef52ed56-8a22-4d15-9f12-a1796ccf17b6`). Apr 26 terminate-guard correctly silently kills any enrollment in these — meaning they look "active" but can't fire. Adam decides per campaign: archive (`UPDATE drip_campaigns SET status='archived'`) or author content. Realtor Relationships build now reduces to: register UUID in `authored-emails.ts` + 4 emails + n8n trigger wire (~1.5 hr post-criteria call).
