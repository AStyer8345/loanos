...
export async function loadOperationalSnapshot(ctx: {db: SupabaseClient;organizationId:string;userId:string}, includePrivate = true): Promise<Snapshot> {
    const fields: Record<string, [
        ...
        const pairs = await Promise.all(Object.entries(fields).filter(([key]) => includePrivate || !['compensation','activity','outbound'].includes(key)).map(async ([key, [table, columns, order]]) => [key, await collectPages(async (from, to) => { const r = await ctx.db.from(table).select(columns).eq('organization_id', ctx.organizationId).order(order || 'id').range(from, to); return { data: r.data, error: r.error }; })]));
        const snapshot = { compensation: [], activity: [], outbound: [], ...Object.fromEntries(pairs) };
    ...
    return { asOf: new Date().toISOString(), viewerId: ctx.userId, organizationId: ctx.organizationId, ...snapshot } as Snapshot;
}
