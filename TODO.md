...
- [ ] **NEEDS ADAM — decide on n8n Drip Scheduler `LqBb3YDLjS2eUrDE`.** Currently active but its "Send via Outlook" node is dead (Microsoft Graph removed). 0 active `drip_enrollments`, 0 sends in last 30 days — no user-visible breakage today. Three options: (a) deactivate until 6 non-PA/DPA campaigns (Ghost Referral, Incomplete App, Went Quiet, Long-Term Nurture, Past Client Retention, Realtor Relationships) actually need to enroll; (b) retarget the Outlook node → Resend HTTP (SDK rewrite via n8n MCP, ~12 nodes); (c) move those 6 campaigns to Workflow DevKit too and archive the scheduler. Pick one before LO #2 onboards.
...
- [ ] **Wire n8n code nodes to fetch templates from `automation_registry` at runtime.** Currently seeded templates are read-only previews — runtime still uses hardcoded copies inside JS code nodes. Per workflow: HTTP GET `automation_registry?name=eq.X`, substitute `{{var}}` tags, send. Also swaps the dead Outlook draft node for Resend at the same time since we're editing the sender anyway. **Refi Intake caveat**: the template uses `{{cash_label}}`, `{{cash_amt}}`, `{{escrow_row_html}}`, `{{processor_note}}` as pre-resolved derived vars — caller must compute these (conditionals on `cash_to_close` sign, `escrow === 'Waived'`, and org-id = Adam) before substitution. **Pre-Approval caveat**: `{{lo_*}}` + `{{brand_header}}` + `{{calendly_link}}` require the same profiles/organizations/org_settings lookup the current JS does. **All 5 extractor workflows** also need a subject-line source — either add `automation_registry.subject_template` column or keep subjects inline in n8n until dashboard UI supports editing them.
- [ ] **`automation_registry` schema gap — Contract Received Party Reply email.** The n8n workflow `UfNcdpoVKQZqy0fj` produces 2 emails (Borrower Welcome + reply-all to transaction parties). Registry row holds 1 template slot, so Party Reply is not seeded. Three options when runtime wiring runs: (a) add `email_template_secondary` column, (b) create a separate `Contract Received — Party Reply` registry row, (c) leave Party Reply as inline n8n code. Pick before wiring task or Contract Received will half-migrate.
