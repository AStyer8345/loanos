...
function mapRow(raw: RawRow, userId: string, organizationId: string): Record<string, unknown> {
  ...
  return {
    ...
    co_borrower_first:      g('First Name (Co Borrower)') ?? g('co_borrower_first'),
    co_borrower_last:       g('Last Name (Co Borrower)')  ?? g('co_borrower_last'),
    email:                  g('Email')       ?? g('email'),
    phone:                  g('Phone')       ?? g('phone'),
    mobile_phone:           g('Mobile')      ?? g('mobile_phone'),
    contact_type:           g('contact_type') ?? 'borrower',
    stage:                  normalizeContactStage(g('Stage') ?? g('stage')),
    ...
    co_borrower_birthdate: parseDate(g('Birthday (Co Borrower)') ?? g('co_borrower_birthdate')),
    co_borrower_mobile:   g('Mobile (Co Borrower)')     ?? g('co_borrower_mobile'),
