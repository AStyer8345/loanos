...
1. **Phone field consolidation:** `phone`, `phone_mobile`, and `home_phone` all exist. For new records going forward, should LoanOS have two phone fields (mobile + direct/office) or just one? The current `phone` field is treated as primary — should we rename it `mobile` to match reality, or keep generic? This affects all future imports and n8n automations.
   ...
   **ANSWERED (2026-03-27):** One phone field for all records. Keep the generic `phone` column. Copy `phone_mobile` and `home_phone` into `phone` where phone is null (consolidates ~106 records). UI hides `phone_mobile` and `home_phone`. Schema columns remain (safe for rollback). Implementation: Migration 060.
...
2. **Realtor stage semantics:** 1,043 realtors are in stage "Lead" because the stage trigger is borrower-centric. Should realtors have a separate stage system (e.g., "Prospecting", "Active Partner", "Inactive", "Churned") or is a simple active/inactive boolean sufficient?
   ...
   **ANSWERED (2026-04-08):** Already resolved. Migration 061 added `realtor_stage` column to contacts. Current data: 111 realtors are "Active Partner" / tier A, 6 are "Active Partner" / tier B, and 955 realtors have null realtor_stage (default — these are the old "Lead" stage realtors that haven't been triaged yet). The `stage` field still holds the borrower pipeline value; `realtor_stage` is the separate realtor-specific lifecycle field. **No further decision needed — implementation exists. Remaining work: backfill realtor_stage for the 955 null records, which is a data quality task, not a schema decision.**
...
3. **`closing_date` on contact vs. loan:** The contact record has a `closing_date` field (53 records populated). Loan records also have closing_date. These mean different things. Should the contact-level closing_date be deprecated in favor of always pulling from the linked loan? Or does it serve a purpose (e.g., tracking a future closing date before the loan is in Arive)?
   ...
   **ANSWERED (2026-04-08):** Deprecate contact-level closing_date. The loans table has 740 "Closed" + 35 "funded" records with closing_date populated — this is the authoritative source. The 53 contact-level closing_dates are legacy Salesforce artifacts. The contact detail page already shows linked loans with their closing dates. **Recommendation: hide closing_date from the contact UI. To surface "most recent close date" on the contact record, compute it via a LEFT JOIN to loans (MAX closing_date WHERE status IN ('Closed','funded')). Do not delete the column yet — leave for schema cleanup phase.**
...
4. **Past client refi trigger:** To build the "Past Clients — Refi Watch" smart list with rate comparison, we need to store the borrower's current/closed rate on either the contact or the loan record. Is this data in Arive via the sync workflow? Should it be pulled automatically, or does Adam want to set it manually?
   ...
   **ANSWERED (2026-04-08):** The `interest_rate` column already exists on the loans table and is well-populated: 710 of 740 "Closed" loans and 35 of 35 "funded" loans have interest_rate values (96% population rate). This data comes from Arive via WF1/WF2 sync workflows — it's automatic, not manual. **For the Refi Watch smart list: JOIN contacts → loans WHERE status IN ('Closed','funded') AND interest_rate > (current_market_rate + threshold). No new column needed. The FRED API integration (pending Adam's API key registration) will provide the current market rate for comparison.**
...
5. **`do_not_call` field:** Before adding any SMS automation via n8n, a `do_not_call` boolean should be added to the contacts schema. Should this be added now (proactively) or when SMS automation is actually being built?
   ...
   **ANSWERED (2026-04-08):** Add now, proactively. iMessage integration is already active (n8n workflow nccX5ml82mMGyE9T logs iMessages to activity_log). The TCPA consent checkboxes are already on website forms (tcpa_consent, sms_opt_in). Adding `do_not_call` now ensures the field exists before any automated outbound messaging is enabled. **Implementation: add `do_not_call BOOLEAN NOT NULL DEFAULT FALSE` to contacts in the next migration. Gate all n8n SMS/call workflows on `do_not_call = false`. This is a compliance requirement, not a feature decision.** ⚠️ ADAM DECISION: Does this also need a `sms_opt_in` boolean on the contact record (separate from the website form field), or is `do_not_call` sufficient as a single block-all flag?
...
6. **Realtor production tiers:** The current schema has `top_realtor` (boolean) and `target_realtor` (boolean). Would a single `production_tier` field (A/B/C or 1/2/3) replace these two booleans more cleanly, or do the two booleans serve different strategic purposes (top = already sending deals, target = want their deals)?
   ...
   **ANSWERED (2026-04-08):** Already resolved. Migration 061 added `production_tier` TEXT column and migration 062 dropped the boolean `top_realtor` and `target_realtor` columns. Current data: 111 realtors are tier A, 6 are tier B, 955 are null (untiered). **No further decision needed — the single production_tier field replaced both booleans. Remaining work: tier the 955 untiered realtors. This is Adam's manual classification task — agent cannot determine business relationship quality.**
...
7. **`last_touch_at` backfill:** Only 648 contacts (27%) have this field populated. Should we run a backfill query that sets `last_touch_at` from `last_activity_date` or `created_at` for the 1,729 contacts missing it? Or leave it null until a real interaction occurs?
   ...
   **ANSWERED (2026-04-08):** Current state: 675 contacts have last_touch_at, 1,820 are missing it. Only 5 of those 1,820 have last_activity_date populated — so backfilling from last_activity_date covers almost nothing. **Recommendation: Do NOT backfill from created_at — that would give every contact a fake "touch" date that doesn't represent real interaction. Leave null. Instead, going forward, ensure every contact_activity insert and every inbound email match triggers an update_last_touch call (this already exists in the contact detail page via updateLastTouch()). The null values will naturally fill as real interactions occur. For the "Realtors Not Touched in 30+ Days" smart list, treat NULL as "never touched" and include those contacts.** No Adam decision needed.
...
8. **Contact dedup on `realtor_email`/`realtor_phone`:** There are 2 contacts with `realtor_email` or `realtor_phone` populated (legacy field). These likely represent borrowers whose realtor was saved incorrectly. Should these be cleaned up and the fields dropped from the UI entirely?
   ...
   **ANSWERED (2026-04-08):** Yes — drop from UI entirely. The `realtor_email` and `realtor_phone` columns are conceptually wrong (realtors have their own contact records with their own email/phone). Only 1-2 records have values. **Implementation: hide both fields from the contact detail UI and contact list view. Do not drop the schema columns yet (safe rollback). The 1-2 populated records should be manually reviewed by Adam to determine if the data belongs on the linked realtor's contact record.** No blocking decision needed — can hide immediately.
