...
- [ ] **Populate `automation_registry` transactional templates — 2/7 done (2026-04-16 PM).** Seeded: Referral Intro, Review Request (both flipped to `fixed_template` — their n8n workflows use hardcoded HTML, not AI). Still empty: Pre-Approval, Final CD, Contract Received, New Application Received, Refi Intake. All 5 are n8n-extractor + hardcoded-HTML workflows — seed the HTML into `email_template` with `{{var}}` merge tags + flip `email_mode` to `fixed_template`. Final CD first (most compliance-sensitive — TRID 3-day + wire-fraud warning in current template).
- [ ] **Wire n8n code nodes to fetch templates from `automation_registry` at runtime.** Currently seeded templates are read-only previews — runtime still uses hardcoded copies inside JS code nodes. Per workflow: HTTP GET `automation_registry?name=eq.X`, substitute `{{var}}` tags, send. Also swaps the dead Outlook draft node for Resend at the same time since we're editing the sender anyway.
