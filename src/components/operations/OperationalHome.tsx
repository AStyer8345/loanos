...
import { buildRows, filterRows, rowTotals, cohortMetrics, periodEvents, previousRange, average, LABELS, evidenceDate, day } from '@/lib/operations/model';
...
const COLUMNS = { receivedAt: 'Date received', amount: 'Loan amount', gross: 'Recorded gross', product: 'Product', source: 'Source', owner: 'Owner', stage: 'Stage', nextAction: 'Next action', dueAt: 'Due', loNeeded: 'Adam needed', email: 'Email', phone: 'Phone', priority: 'Priority', sourcePage: 'Originating page', referral: 'Referral partner' };
type Props = {
...
export default function OperationalHome({ request = defaultRequest, recordBase = '', title = 'Command Center' }: Props) {
    const [data, setData] = useState<Snapshot | null>(null), [error, setError] = useState(''), [notice, setNotice] = useState(''), [busy, setBusy] = useState(false), [view, setView] = useState('Today'), [mode, setMode] = useState('cohort'), [filters, setFilters] = useState<Filters>(ALL_FILTERS), [selected, setSelected] = useState<Row | null>(null), [drill, setDrill] = useState<{
        label: string;
        ids: string[];
    } | null>(null), [columns, setColumns] = useState<string[]>(['receivedAt', 'amount', 'gross', 'product', 'source', 'owner', 'stage', 'nextAction']), [columnsOpen, setColumnsOpen] = useState(false);
    ...
    const tableCell = (r: Row, k: string) => k === 'priority' ? (r.preference?.priority_follow_up ? 'Priority' : '—') : k === 'amount' ? money(r.amount) : k === 'gross' ? money(r.gross) : k === 'receivedAt' ? date(r.receivedAt) : k === 'dueAt' ? date(r.dueAt) : k === 'loNeeded' ? (r.loNeeded ? 'Yes · escalated' : 'No') : (nice(String(r[k as keyof Row] || '—')));
  ...
  {view === 'Today' && <p className="ops-caption">Open assigned work and saved priorities. Waiting work stays with its task owner. “Adam needed” requires an explicit escalation; assignment alone does not imply a loan officer decision.</p>}
