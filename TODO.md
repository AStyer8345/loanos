...
- [ ] **Populate `automation_registry` transactional templates.** 5 rows have `email_mode='hybrid'` (Pre-Approval, Final CD, Contract Received, New Application Received, Refi Intake) and 2 have `email_mode='ai_generated'` (Referral Intro, Review Request), but all have empty `email_template`. Confirm whether `/dashboard/automations` renders + saves these, and whether Workflow DevKit should read templates from there at runtime, before seeding content. Needs a separate scoped pass.
