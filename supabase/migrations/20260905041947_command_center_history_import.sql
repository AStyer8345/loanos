-- Historical source evidence is imported without replaying old communications.
BEGIN;
CREATE TABLE public.lead_desk_preferences (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES public.organizations(id),
 legacy_key text, contact_id uuid, inquiry_id uuid,
 status text, notes text NOT NULL DEFAULT '', priority_follow_up boolean NOT NULL DEFAULT false,
 amount_note text, product_note text, hidden boolean NOT NULL DEFAULT false,
 source_updated_at timestamptz, updated_at timestamptz NOT NULL DEFAULT now(),
 match_state text NOT NULL DEFAULT 'review' CHECK(match_state IN ('matched','review')),
 provenance jsonb NOT NULL DEFAULT '{}',
 UNIQUE(organization_id,legacy_key), UNIQUE(organization_id,contact_id),
...
ALTER TABLE public.lead_desk_preferences ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.lead_desk_preferences FROM PUBLIC,anon,authenticated;
GRANT SELECT ON public.lead_desk_preferences TO authenticated;
GRANT ALL ON public.lead_desk_preferences TO service_role;
CREATE POLICY lead_desk_preferences_read ON public.lead_desk_preferences FOR SELECT TO authenticated USING(organization_id=(select public.get_my_organization_id()));
-- Updates are validated by the authenticated operational endpoint. No financial fields are exposed.
GRANT UPDATE(status,notes,priority_follow_up,hidden,updated_at) ON public.lead_desk_preferences TO authenticated;
CREATE POLICY lead_desk_preferences_update ON public.lead_desk_preferences FOR UPDATE TO authenticated USING(organization_id=(select public.get_my_organization_id())) WITH CHECK(organization_id=(select public.get_my_organization_id()));
...
BEGIN
 ...
 END IF;
 IF coalesce(p_input->>'historical_import','false')<>'true' THEN
