...
- [ ] **Drip end-to-end execution gap — prod deploy BLOCKED.** Code shipped 2026-04-23 AM (`authored-emails.ts`, `/api/drip/run`, enrollment POST). But `vercel.json` cron `0 * * * *` is **Pro-plan only** — Hobby tier caps schedules at daily. Every prod deploy since `dcbbe25` has been rejected pre-build with `vercel.link/3Fpeeb1` (cron-pricing page). Seven commits on `main` (through MISMO merge `16d9021` + nudge `b23f8b4`) have not reached production. **NEEDS ADAM decision:** (a) upgrade Vercel project to Pro, (b) change cron to daily `0 13 * * *` (loses intraday drip precision), or (c) move scheduler back to n8n and drop `vercel.json` crons. Once fixed, also set `CRON_SECRET` env var.
