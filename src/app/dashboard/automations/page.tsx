  ...
  return (
    ...
    >
      <div className="bg-zinc-900 border border-zinc-700 border-l-[3px] border-l-amber-500 rounded-r-lg p-7 w-full max-w-lg relative max-h-[90vh] overflow-y-auto">
        ...
        {(phase === 'upload' || phase === 'extracting') && (
          <>
            ...
            >
              ...
              {file && <div className="text-xs text-zinc-500 mt-1 font-mono">{(file.size / 1024).toFixed(0)} KB</div>}
            </div>
            <input
              ref={fileRef}
              type="file"
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-zinc-400 hover:text-zinc-200 border border-zinc-700 hover:border-zinc-500 rounded-md transition-colors font-mono"
              >
                Back
              </button>
            </div>
        ...
        {/* ── SENDING phase ── */}
        {phase === 'sending' && (
          <div className="flex items-center gap-3 py-4 text-sm text-zinc-500 font-mono">
        ...
        {phase === 'success' && (
          <div className="py-4">
            <div className="text-sm rounded-md px-3 py-2 mb-4 bg-emerald-900/30 border border-emerald-700 text-emerald-400 font-mono">
...
function TriggerModal({ wf, loanId, onClose }: { wf: Workflow; loanId: string | null; onClose: () => void }) {
  ...
  return (
    <div
      ...
      onClick={(e) => { if (e.target === e.currentTarget) onClose() }}
    >
      <div className="bg-zinc-900 border border-zinc-700 border-l-4 border-l-emerald-500 rounded-lg p-7 w-full max-w-md relative shadow-xl">
        {/* Close */}
        <button
          onClick={onClose}
          className="absolute top-3 right-4 text-xs text-zinc-500 hover:text-zinc-300 font-mono"
        >
          [ESC]
        </button>

        {/* Title */}
        <div className="text-lg font-semibold text-zinc-100 mb-0.5">{wf.name}</div>
        <div className="text-xs text-zinc-500 font-mono mb-5">
        ...
        {wf.triggerType === 'form' && (
          <div className="flex flex-col gap-3 mb-4">
            {([
              ...
              <div key={label}>
                <label className="block text-xs font-medium text-zinc-500 font-mono mb-1">{label}</label>
                <input
                  value={value}
                  onChange={(e) => set(e.target.value)}
                  placeholder={placeholder}
                  className="w-full bg-zinc-800 border border-zinc-600 rounded-md px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 font-mono placeholder-zinc-600"
                />
              </div>
            ))}
            <div>
              <label className="block text-xs font-medium text-zinc-500 font-mono mb-1">Details</label>
              <textarea
                value={details}
                onChange={(e) => setDetails(e.target.value)}
                placeholder="Buying in Austin, pre-approved $450k, relocating from Dallas..."
                rows={3}
                className="w-full resize-y bg-zinc-800 border border-zinc-600 rounded-md px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 font-mono placeholder-zinc-600"
              />
            </div>
          </div>
        )}
...
function AutoCard({ wf, index, loans, onTrigger }: { wf: Workflow; index: number; loans: LoanOption[]; onTrigger: (loanId: string | null) => void }) {
  ...
  return (
    <div
      className="auto-card bg-zinc-900 border border-zinc-700 rounded-lg p-6 relative overflow-hidden hover:border-zinc-500 transition-all"
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        {/* Status badge */}
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-900/30 border border-emerald-700 shrink-0">
      ...
      <div className="flex items-center mb-5">
        {STEPS.map((step, i) => (
          <div key={step.key} className="flex items-center" style={{ flex: i < STEPS.length - 1 ? undefined : 1 }}>
            ...
            {i < STEPS.length - 1 && (
              <div className="flow-connector flex-1 h-px bg-zinc-700 relative overflow-visible mb-3.5">
                <div
                  className="flow-dot"
      ...
      <div className="meta-reveal text-[10px] text-zinc-600 font-mono flex gap-4 mb-4">
        <span>n8n: <span className="text-zinc-400">{wf.n8nId || '—'}</span></span>
        <span>webhook: <span className="text-zinc-400">/webhook/{wf.webhookPath}</span></span>
      ...
      <div className="flex items-center gap-2">
        <select
          ...
          className="flex-1 text-xs bg-zinc-800 border border-zinc-600 rounded-md px-2 py-1.5 text-zinc-300 focus:outline-none focus:border-emerald-500 font-mono"
        >
...
export default function AutomationsPage() {
  ...
  return (
    <>
      ...
      <div className="p-8 pb-12 bg-zinc-950 min-h-full">
        ...
        <div className="flex border border-zinc-700 rounded-lg overflow-hidden bg-zinc-900 mb-6">
          {[
            ...
            <div key={stat.label} className={`flex-1 px-5 py-4 ${i < 3 ? 'border-r border-zinc-700' : ''}`}>
        ...
        {/* ── Infra status bar ────────────────────────────────────────── */}
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-900/30 border border-emerald-700 mb-8">
        ...
        {/* ── Footer note ─────────────────────────────────────────────── */}
        <div className="mt-8 px-4 py-3 bg-zinc-900 border border-zinc-700 rounded-lg text-xs text-zinc-500 font-mono">
