...
### Verify audit findings before acting on them (2026-03-25)
The 2026-03-24 audit reported 6 files with `bg-white` dark theme violations (`TopNav.tsx`, `NavDropdown.tsx`, `NavItem.tsx`, `ScenarioCard.tsx`, `StatementUpload.tsx`, `ScenarioList.tsx`). Manual verification on 2026-03-25 found **zero** `bg-white` occurrences in any of these files — they all use dark CSS variables or zinc-based Tailwind classes. The original audit likely used a stale grep or matched on content that was already fixed. Pattern: always `grep` the actual file before logging a violation. Audit metrics that are not independently verified can propagate false positives across runs.

### Stale loan metrics require case-insensitive status matching (2026-03-25)
The stale loan query `status NOT IN ('funded','denied','withdrawn','cancelled','dead','closed')` returned 824/853 loans because Supabase stores status as-is from Arive: `Closed` (capital C), `APPLICATION_INTAKE`, `CLEAR_TO_CLOSE`, etc. Case-sensitive `NOT IN` misses these. Pattern: any query that filters on status values must use `lower(status)` or `ILIKE` until a status normalization migration is applied.
